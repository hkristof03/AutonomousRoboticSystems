...
class Wall(object):
    ...
    def draw(self):
        pygame.draw.line(win, (0, 0, 0), (self.x1, self.y1), (self.x2, self.y2), 5)
...
class Robot(object):
    ...
    def draw(self):
        ...
        textPos = (self.x + (math.cos(self.theta) - 0.15) * textDistance,
                   self.y + (math.sin(self.theta)) * textDistance - math.sin(self.theta) * 5 - 5)
        text = self.font.render(str(currAngle), 1, BLACK)
        ...
        lineEnd = (math.floor(self.x) + math.floor(outerRad * math.cos(self.theta)),
                    math.floor(self.y) + math.floor(outerRad * math.sin(self.theta)))
        ...
        for sen in self.sensors:
            pygame.draw.line(win, (255, 0, 0), (sen.x1, sen.y1), (sen.x2, sen.y2), sen.width)
            text_ = self.font.render(str(sen.distance), 1, BLUE)
            win.blit(text_, (sen.x1, sen.y1))

        #WHEELS
        textPosL = (self.x + (math.cos(self.theta + math.radians(-90)) - 0.15) * textDistance,
                   self.y + (math.sin(self.theta + math.radians(-90))) * textDistance - math.sin(self.theta) * 5 - 5)
