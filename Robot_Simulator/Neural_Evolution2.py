...
class GeneticAlgorithm(object):
    ...
    def GAPipeLine(self, individuals, proportion):
        ...
        while i_ < ck:
            ...
            i_ = i_ + step
            print("i_ at end:", i_)
            print("parent_1 type:", type(parent_1))
            print("parent_2 type:", type(parent_2))
            print("size of parent_1:", parent_1.size)
            print("size of parent_2:", parent_2.size)
        mutated_offsprings = []
...
class Robot(object):
    ...
    def fitness_function_2(self):
        ...
        self.velocityScore += (abs(self.velocity)) * (1 - col)  # total positive score from the velocity
        self.fitnessScore = (dust_weight * self.dustEaten + vel_weight * self.velocityScore) / (
                    coll_weight * self.collisionScore + 1)
...
while run:
    ...
    for robot in robots:
        ...
        dt = 0
        terminate = False
        while (dt < 1000) and (terminate == False):
            bias_x = 0
            bias_y = 0
            d_theta = 0
    ...
    robots, df = run_GA(robots, proportion, df, GA, file_name, num_generation)
